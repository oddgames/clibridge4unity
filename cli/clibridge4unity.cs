...
// Example: clibridge4unity ANALYZE BridgeServer
using System;
...
class Program
{
    ...
    static int Main(string[] args)
    {
        ...
        // SCREENSHOT is handled CLI-side using Win32 screen capture
        // All pipe communication happens HERE - HandleScreenshot never opens pipes
        if (command.Equals("SCREENSHOT", StringComparison.OrdinalIgnoreCase))
        {
            string[] screenshotParts = (data ?? "").Trim().Split(' ', 2, StringSplitOptions.RemoveEmptyEntries);
            string view = screenshotParts.Length > 0 ? screenshotParts[0].ToLowerInvariant() : "editor";

            // "list" just forwards to WINDOWS
            if (view == "list")
                return SendCommand(pipeName, projectPath, "WINDOWS", "");

            // For specific views, fetch window layout data upfront (single pipe connection)
            string windowsData = null;
            if (view != "editor")
            {
                windowsData = SendCommandGetResponse(pipeName, "WINDOWS", "");
                if (string.IsNullOrEmpty(windowsData))
                {
                    Console.Error.WriteLine("Error: Could not get window list from Unity (empty response)");
                    return 1;
                }
            }

            return HandleScreenshot(projectPath, data, windowsData);
        }
    ...
    static void PrintUsage()
    {
        ...
        Console.Error.WriteLine("  clibridge4unity ANALYZE BridgeServer       # Analyze a class");
    ...
    /// <summary>
    /// Handles screenshot capture. windowsData is pre-fetched WINDOWS response (null for "editor" view).
    /// This method NEVER opens pipe connections - all Unity communication happens in the caller.
    /// </summary>
    static int HandleScreenshot(string projectPath, string data, string windowsData)
    {
        string[] parts = (data ?? "").Trim().Split(' ', 2, StringSplitOptions.RemoveEmptyEntries);
        string view = parts.Length > 0 ? parts[0].ToLowerInvariant() : "editor";
        string outputPath = parts.Length > 1 ? parts[1].Trim() : null;

        // Find Unity's window handle
        string projectName = Path.GetFileName(Path.GetFullPath(projectPath));
        ...
        foreach (var proc in Process.GetProcessesByName("Unity"))
        {
            if (proc.MainWindowHandle != IntPtr.Zero &&
                proc.MainWindowTitle.Contains(projectName, StringComparison.OrdinalIgnoreCase))
            {
                unityHwnd = proc.MainWindowHandle;
                break;
            }
        }
        ...
        // For specific views (not "editor"), get window positions from Unity
        int captureX = 0, captureY = 0, captureW = 0, captureH = 0;

        if (view != "editor")
        {
            // Use pre-fetched window layout data (no pipe connections in this method)
            try
            {
                // Map shorthand names to Unity type names
                string targetType = view switch
                {
                    "game" => "GameView",
                    "scene" => "SceneView",
                    ...
                    _ => view // Try as-is for custom window titles
                };

                bool found = false;
                bool isHidden = false;
                foreach (var line in windowsData.Split('\n'))
                {
                    var trimmed = line.Trim();
                    ...
                    // Match by type name or window title
                    if (fields[1].Contains(targetType, StringComparison.OrdinalIgnoreCase) ||
                        fields[0].Contains(view, StringComparison.OrdinalIgnoreCase))
                    {
                        captureX = int.Parse(fields[2]);
                        captureY = int.Parse(fields[3]);
                        captureW = int.Parse(fields[4]);
                        captureH = int.Parse(fields[5]);
                        isHidden = fields.Length > 6 && fields[6].Trim() == "hidden";
                        found = true;
                        break;
                    }
                }
                ...
                if (!found)
                {
                    Console.Error.WriteLine($"Error: Window '{view}' not found. Use 'SCREENSHOT list' to see available windows.");
                    return 1;
                }

                if (isHidden)
                {
                    Console.Error.WriteLine($"Warning: '{view}' is behind another tab and may not capture correctly.");
                }
            }
            catch (Exception ex)
            {
                Console.Error.WriteLine($"Error getting window layout: {ex.Message}");
                return 1;
            }
        }
        else
        {
            // "editor" - full window, no crop needed (handled later)
        }

        if (view != "editor" && (captureW <= 0 || captureH <= 0))
        {
            Console.Error.WriteLine($"Error: Invalid capture size ({captureW}x{captureH})");
            return 1;
        }

        // Generate output path - use temp directory, overwrite by view name
        if (string.IsNullOrEmpty(outputPath))
        {
            string screenshotDir = Path.Combine(Path.GetTempPath(), "clibridge4unity_screenshots");
            if (!Directory.Exists(screenshotDir))
                Directory.CreateDirectory(screenshotDir);
            outputPath = Path.Combine(screenshotDir, $"{view}.png");
        }
        ...
        // Use PrintWindow to capture Unity without needing foreground focus.
        // EditorWindow.position returns screen-absolute coordinates, so we convert
        // to window-relative by subtracting the window's screen position.
        GetWindowRect(unityHwnd, out RECT winRect);
        int winW = winRect.Right - winRect.Left;
        int winH = winRect.Bottom - winRect.Top;

        IntPtr winDC = GetDC(unityHwnd);
        IntPtr memDC = CreateCompatibleDC(winDC);
        IntPtr bitmap = CreateCompatibleBitmap(winDC, winW, winH);
        IntPtr oldBitmap = SelectObject(memDC, bitmap);

        PrintWindow(unityHwnd, memDC, 2);
        ...
        if (view == "editor")
        {
            ...
            pixels = new byte[winW * winH * 4];
            GetDIBits(memDC, bitmap, 0, (uint)winH, pixels, ref bmi, 0);
        }
        else
        {
            // Convert screen-absolute EditorWindow position to window-relative
            int srcX = captureX - winRect.Left;
            int srcY = captureY - winRect.Top;

            // Check if the view is within the Unity window bounds
            if (srcX < 0 || srcY < 0 || srcX + captureW > winW || srcY + captureH > winH)
            {
                Console.Error.WriteLine($"Warning: '{view}' appears to be outside the main Unity window (floating/other monitor).");
                Console.Error.WriteLine($"  Window: ({winRect.Left},{winRect.Top})-({winRect.Right},{winRect.Bottom})");
                Console.Error.WriteLine($"  View: ({captureX},{captureY}) size {captureW}x{captureH}");
                // Clamp to available area
                if (srcX < 0) { captureW += srcX; srcX = 0; }
                if (srcY < 0) { captureH += srcY; srcY = 0; }
                if (srcX + captureW > winW) captureW = winW - srcX;
                if (srcY + captureH > winH) captureH = winH - srcY;
                if (captureW <= 0 || captureH <= 0)
                {
                    SelectObject(memDC, oldBitmap);
                    DeleteObject(bitmap);
                    DeleteDC(memDC);
                    ReleaseDC(unityHwnd, winDC);
                    Console.Error.WriteLine("Error: View is completely outside the Unity window. Cannot capture.");
                    return 1;
                }
            }

            // Crop from the full window capture
            IntPtr cropDC = CreateCompatibleDC(winDC);
            IntPtr cropBitmap = CreateCompatibleBitmap(winDC, captureW, captureH);
            IntPtr cropOld = SelectObject(cropDC, cropBitmap);
            BitBlt(cropDC, 0, 0, captureW, captureH, memDC, srcX, srcY, SRCCOPY);
            ...
            pixels = new byte[captureW * captureH * 4];
            GetDIBits(cropDC, cropBitmap, 0, (uint)captureH, pixels, ref bmi, 0);
        ...
        WritePng(outputPath, captureW, captureH, pixels);

        string fullPath = Path.GetFullPath(outputPath);
        Console.WriteLine($"path: {fullPath}");
        Console.WriteLine($"view: {view}");
        Console.WriteLine($"width: {captureW}");
        Console.WriteLine($"height: {captureH}");
        return 0;
    }
    ...
    static int HandleInstall(string pipeName, string projectPath, string data)
    {
        ...
        md.AppendLine();
        md.AppendLine("This project has `clibridge4unity` installed for Unity Editor automation.");
        md.AppendLine("Use the CLI to communicate with the running Unity Editor via named pipes.");
        md.AppendLine();
        md.AppendLine("## Quick Start");
        md.AppendLine();
        md.AppendLine("```bash");
        md.AppendLine("# Basic commands (run from project directory)");
        md.AppendLine("clibridge4unity PING                    # Test connection");
        md.AppendLine("clibridge4unity STATUS                  # Editor state, compile status");
        md.AppendLine("clibridge4unity HELP                    # List all commands from Unity");
        md.AppendLine("clibridge4unity COMPILE                 # Force recompilation (waits for finish)");
        md.AppendLine("clibridge4unity LOG errors              # Show Unity console errors");
        md.AppendLine("```");
        md.AppendLine();
        md.AppendLine("## CRITICAL Rules");
        md.AppendLine();
        md.AppendLine("1. **Always use the `clibridge4unity` binary directly** — never `dotnet run`");
        md.AppendLine("2. **After editing Package code, run `COMPILE` and verify it succeeds** before testing");
        md.AppendLine("3. **Check `LOG errors` when commands behave unexpectedly** — Unity may have compile errors");
        md.AppendLine("4. **Use single-quoted JSON** in bash to avoid variable expansion issues");
        md.AppendLine("5. **SCREENSHOT saves to %TEMP%** — never in the project directory");
        md.AppendLine();
        md.AppendLine("## Component Manipulation");
        md.AppendLine();
        md.AppendLine("```bash");
        md.AppendLine("# Inspect components on a GameObject");
        md.AppendLine("clibridge4unity INSPECTOR 'Canvas/Panel'");
        md.AppendLine("clibridge4unity INSPECTOR '{\"gameObject\":\"Canvas/Panel\",\"component\":\"Image\"}'");
        md.AppendLine();
        md.AppendLine("# Set component field values");
        md.AppendLine("clibridge4unity COMPONENT_SET '{\"gameObject\":\"Panel\",\"component\":\"Image\",\"field\":\"m_Color\",\"value\":\"#FF0000\"}'");
        md.AppendLine();
        md.AppendLine("# Add/remove components");
        md.AppendLine("clibridge4unity ADDCOMPONENT Panel BoxCollider");
        md.AppendLine("clibridge4unity REMOVECOMPONENT Panel BoxCollider");
        md.AppendLine("```");
        md.AppendLine();
        md.AppendLine("## Screenshots & Rendering");
        md.AppendLine();
        md.AppendLine("```bash");
        md.AppendLine("# Window screenshots (CLI-side, works when Unity is in background)");
        md.AppendLine("clibridge4unity SCREENSHOT editor     # Full editor window");
        md.AppendLine("clibridge4unity SCREENSHOT scene      # Scene view only");
        md.AppendLine("clibridge4unity SCREENSHOT game       # Game view only");
        md.AppendLine("clibridge4unity SCREENSHOT inspector  # Inspector panel");
        md.AppendLine("```");
        md.AppendLine();
        md.AppendLine("## Code Analysis");
        md.AppendLine();
        md.AppendLine("```bash");
        md.AppendLine("clibridge4unity SEARCH 'class:PlayerController'    # Find classes");
        md.AppendLine("clibridge4unity SEARCH 'method:Update'             # Find methods");
        md.AppendLine("clibridge4unity SEARCH 'inherits:MonoBehaviour'    # Find by inheritance");
        md.AppendLine("clibridge4unity ANALYZE 'PlayerController'         # Full class analysis");
        md.AppendLine("clibridge4unity ANALYZE 'PlayerController.Move'    # Member details");
        md.AppendLine("```");
        md.AppendLine();
        md.AppendLine("## Scene Commands");
        md.AppendLine();
        md.AppendLine("```bash");
        md.AppendLine("clibridge4unity SCENE                  # Scene info + hierarchy");
        md.AppendLine("clibridge4unity CREATE 'MyObject'      # Create GameObject");
        md.AppendLine("clibridge4unity FIND 'Player'          # Find by name");
        md.AppendLine("clibridge4unity DELETE 'TempObject'    # Delete GameObject");
        md.AppendLine("clibridge4unity SAVE                   # Save current scene");
        md.AppendLine("clibridge4unity LOAD 'Assets/Scenes/Main.unity'");
        md.AppendLine("clibridge4unity PLAY                   # Enter play mode");
        md.AppendLine("clibridge4unity STOP                   # Exit play mode");
        md.AppendLine("```");
        md.AppendLine();
        md.AppendLine("## Prefab Commands");
        md.AppendLine();
        md.AppendLine("```bash");
        md.AppendLine("clibridge4unity PREFAB_CREATE '{\"name\":\"MyPrefab\",\"path\":\"Assets/Prefabs\"}'");
        md.AppendLine("clibridge4unity PREFAB_HIERARCHY 'Assets/Prefabs/MyPrefab.prefab'");
        md.AppendLine("clibridge4unity PREFAB_INSTANTIATE '{\"prefab\":\"Assets/Prefabs/MyPrefab.prefab\"}'");
        md.AppendLine("```");
        md.AppendLine();
        md.AppendLine("## Troubleshooting");
        md.AppendLine();
        md.AppendLine("- **\"Main thread timed out\"**: Unity is in background. CLI auto-retries with WAKEUP.");
        md.AppendLine("- **Command not found after code change**: Run `clibridge4unity COMPILE` first.");
        md.AppendLine("- **Connection refused**: Check Unity is running and console shows `[Bridge] Server started`.");
        md.AppendLine("- **Use `WAKEUP` manually**: `clibridge4unity WAKEUP` — brings Unity to foreground briefly.");
        md.AppendLine("- **Fuzzy matching**: Paths like `\"Button\"` auto-match `\"Canvas/Panel/Button\"`. Ambiguous matches return errors.");
        md.AppendLine();
