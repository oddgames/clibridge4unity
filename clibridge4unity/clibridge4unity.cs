...
class Program
{
    ...
    static int HandleInstall(string pipeName, string projectPath, string data)
    {
        ...
        md.AppendLine();
        md.AppendLine("## Compile & Play Workflow");
        md.AppendLine();
        md.AppendLine("**CRITICAL: Unity cannot compile during play mode.** The workflow is:");
        md.AppendLine("1. `STOP` — exit play mode (if playing)");
        md.AppendLine("2. Edit .cs files");
        md.AppendLine("3. `COMPILE` — triggers Unity recompilation (waits for completion)");
        md.AppendLine("4. Check STATUS for compile errors — fix before proceeding");
        md.AppendLine("5. `PLAY` — enter play mode to test");
        md.AppendLine();
        md.AppendLine("```bash");
        md.AppendLine("clibridge4unity STOP                    # Exit play mode first");
        md.AppendLine("clibridge4unity COMPILE                 # Recompile (auto-skips if no changes)");
        md.AppendLine("clibridge4unity STATUS                  # Check for compile errors");
        md.AppendLine("clibridge4unity PLAY                    # Enter play mode");
        md.AppendLine("clibridge4unity PLAY Assets/Scenes/Main.unity  # Play a specific scene");
        md.AppendLine("```");
        md.AppendLine();
        md.AppendLine("CODE_EXEC/CODE_EXEC_RETURN do NOT need COMPILE — they use their own Roslyn compiler.");
        md.AppendLine();
        ...
        md.AppendLine();
        md.AppendLine("```bash");
        md.AppendLine("clibridge4unity TEST list                # List all tests");
        md.AppendLine("clibridge4unity TEST list MyClass         # Filter by name (substring)");
        md.AppendLine("clibridge4unity TEST MyTest               # Run matching tests (streaming results)");
        md.AppendLine("clibridge4unity TEST playmode MyTest      # Run PlayMode tests");
        md.AppendLine("clibridge4unity TEST all                  # Run EditMode + PlayMode");
        md.AppendLine("```");
        md.AppendLine();
        md.AppendLine("Results stream with progress: `[3/17] PASS TestName (0.5s) ~8s left`");
        md.AppendLine("Failed tests show assertion message + stack trace.");
        md.AppendLine();
        md.AppendLine("## Debug & Inspect");
        md.AppendLine();
        md.AppendLine("```bash");
        md.AppendLine("# Inspect result of any expression as a reflection tree");
        md.AppendLine("clibridge4unity CODE_EXEC_RETURN Camera.main --inspect 2");
        md.AppendLine("clibridge4unity CODE_EXEC_RETURN Camera.main --inspect 2 --private");
        md.AppendLine();
        md.AppendLine("# Line-by-line execution trace with variable capture");
        md.AppendLine("clibridge4unity CODE_EXEC_RETURN 'int x=10; x+=5; return x;' --trace");
        md.AppendLine("clibridge4unity CODE_EXEC_RETURN @script.cs --trace --vars x,y");
        md.AppendLine("clibridge4unity CODE_EXEC_RETURN @script.cs --trace --only hasErrors");
        md.AppendLine("```");
        md.AppendLine();
        md.AppendLine("## CLI-side Commands (no Unity pipe needed)");
        md.AppendLine();
        md.AppendLine("```bash");
        md.AppendLine("clibridge4unity WAKEUP                   # Bring Unity to foreground");
        md.AppendLine("clibridge4unity WAKEUP refresh            # Bring to foreground + Ctrl+R recompile");
        md.AppendLine("clibridge4unity DISMISS                   # Close modal dialogs");
