...
class Program
{
    ...
    static int HandleInstall(string pipeName, string projectPath, string data)
    {
        ...
        md.AppendLine();
        md.AppendLine("```bash");
        md.AppendLine("clibridge4unity CODE_EXEC 'Debug.Log(\"hello\")'              # Fire-and-forget");
        md.AppendLine("clibridge4unity CODE_EXEC_RETURN 'return GameObject.FindObjectsOfType<Camera>().Length'");
        md.AppendLine("clibridge4unity CODE_EXEC @script.cs                         # Run from file (no size limit)");
        md.AppendLine("```");
        md.AppendLine();
