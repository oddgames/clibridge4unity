...
class Program
{
    ...
    static int HandleInstall(string pipeName, string projectPath, string data)
    {
        ...
        md.AppendLine("5. **SCREENSHOT saves to %TEMP%** and auto-opens in VS Code");
        md.AppendLine();
        ...
        md.AppendLine("clibridge4unity COMPONENT_ADD Panel BoxCollider");
        md.AppendLine("clibridge4unity COMPONENT_REMOVE Panel BoxCollider");
        md.AppendLine("```");
        md.AppendLine();
        md.AppendLine("## Screenshots & Rendering");
        md.AppendLine();
        md.AppendLine("```bash");
        md.AppendLine("# Screenshot the full editor window (CLI-side PrintWindow, works in background)");
        md.AppendLine("clibridge4unity SCREENSHOT            # Captures editor, auto-opens in VS Code");
        md.AppendLine();
        md.AppendLine("# Render prefabs/GameObjects to PNG (server-side, auto-opens in VS Code)");
        md.AppendLine("clibridge4unity UI_RENDER Assets/Prefabs/MyPrefab.prefab");
        md.AppendLine("```");
        md.AppendLine();
        md.AppendLine("## Code Analysis");
        md.AppendLine();
        md.AppendLine("```bash");
        md.AppendLine("clibridge4unity CODE_SEARCH 'class:PlayerController'    # Find classes");
        md.AppendLine("clibridge4unity CODE_SEARCH 'method:Update'             # Find methods");
        md.AppendLine("clibridge4unity CODE_SEARCH 'inherits:MonoBehaviour'    # Find by inheritance");
        md.AppendLine("clibridge4unity CODE_ANALYZE 'PlayerController'         # Full class analysis");
        md.AppendLine("clibridge4unity CODE_ANALYZE 'PlayerController.Move'    # Member details");
        md.AppendLine("```");
        md.AppendLine();
        ...
        md.AppendLine("```");
        md.AppendLine();
        md.AppendLine("## Stack Traces");
        md.AppendLine();
        md.AppendLine("When you encounter Unity stack traces (from LOG, CODE_EXEC errors, etc.),");
        md.AppendLine("use `STACK_MINIMIZE` to strip internal frames and shorten paths:");
        md.AppendLine();
        md.AppendLine("```bash");
        md.AppendLine("clibridge4unity STACK_MINIMIZE '<paste stack trace>'");
        md.AppendLine("```");
        md.AppendLine();
