...
│   └── package.json           # UPM manifest (v1.1.86)
...
- `VISIBLE x,y,w,h` - What is inside a screen rectangle (physical desktop pixels). Also reachable as `CONTEXT --rect x,y,w,h`, which is how every `CAPTURE` annotates its screenshot. See [RegionVisibility.cs](Package/Editor/Commands/Core/RegionVisibility.cs).
  ...
  - Objects a region resolves to carry their **data**, not just names: components inline per row, plus serialized fields via INSPECTOR. Hierarchy rows resolve through `EntityId` (Unity 6.3 changed `TreeViewItem.id` from `int` to `UnityEngine.EntityId`, which is not IConvertible — `as int?` yields null silently, so every row resolved to id 0 and the selected-row flag was always false).
...
- **Prompt**: a `TaskDialogIndirect` desktop dialog, four plain buttons — *Allow* (run it in the live play session; right for `CODE_EXEC`, which carries its own compiler), *Allow until play stops*, *Always allow*, *Deny* (also Esc / close box). Kept to a two-line body because it lands mid-play-session and must be answerable at a glance; *exit play mode and hand over* lives only in `ALLOW <id> yield`. Not a toast: toasts from an unpackaged single-file exe need an AUMID + Start Menu shortcut and a COM/URI activator, all of which can rot silently.
- **Owner is read from the heartbeat file, not the pipe** — the editor is busy in a play session exactly when we need to ask.
- **Answer from any terminal** (works with no pipe): `REQUESTS` lists what's waiting · `ALLOW <id>` runs it in the live session · `ALLOW <id> yield` exits play mode and hands over · `ALLOW <id> session|always` runs it and stops asking · `DENY <id>`. Requests carry the caller's PID and are swept when that process exits — nobody is waiting for the answer.
- **Grants — the gate asks once, not once per command, and grants are PROJECT-wide.** Asking per command is unusable, not safe: one task can issue a dozen mutating commands. *Allow until play stops* (or `REQUESTS --session`) writes `grants/_session.txt` keyed on `(owner, playOwnerSince)`, so leaving play mode and re-entering invalidates it. *Always allow* (or `REQUESTS --always`) writes `grants/_always.txt`, valid until `REQUESTS --forget`. Neither is per window: a window id is its anchor process's pid, which changes with every conversation and — when the host anchors on a short-lived process — with every command, so per-window grants were never matched again and the gate kept asking. `REQUESTS` reports the standing permission; `CLIBRIDGE_NO_PLAYGATE=1` disables the gate for a window entirely.
...
- `CAPTURE [--out <png>] [--full]` - Drag-select a screen region (Esc/right-click cancels). Freezes the desktop first, then lets you select on the still — so the crop is the frame you aimed at, not whatever Unity repainted mid-drag. Works while Unity is busy, importing, or showing the modal dialog you're trying to screenshot. See [RegionCapture.cs](clibridge4unity/RegionCapture.cs).
  ...
  - **Not over Unity? Plain screenshot.** Decided by Z-ORDER, not rectangles: nine points across the region go through `WindowFromPoint` to find what is actually *visible* there. Rect overlap is not enough — a maximized editor is "under" almost the whole screen, so a shot of VS Code or Chrome sitting on top of Unity came back annotated with Unity's scene. Unity cannot detect this itself; from inside the editor there is no way to know another app is covering it. The bridge's own verdict ("no editor window overlaps") also overrides a match. **Clipboard is image-only by default** (`--clipboard image|text|both|off`, also in the tray menu). Both formats at once sounds ideal but is not: browser-based apps (claude.ai, Slack, Discord) given text and image together paste the *text* in preference, or attach the image and then dump 16 KB of prompt after it — so when the screenshot is the target, text cannot be on the clipboard too. The prompt is one step away: `CAPTURE --last-prompt` (or the tray's *Copy last prompt as text*) swaps the clipboard for the most recent capture's editor state, and it is always in the `.context.md` beside the image. A plain screenshot also reports which window was on top.
  ...
  - `CAPTURE --daemon` - tray icon + global hotkeys: **PrtScn** grabs a region, **Ctrl+PrtScn** starts/stops a recording (one chord toggles — reaching for a different key to stop is when you fumble it). `--status` / `--stop` manage it; `--autostart on|off` writes/removes a Startup-folder `.cmd`. Refuses to double-arm — two daemons means `RegisterHotKey` fails in the loser and the chord silently dies.
    - **Hotkeys are re-armed once a minute** and everything the daemon would have said goes to `captures/daemon.log` (tail shown by `--status`). Its console is hidden, so without the log a dead hotkey and a failed capture both looked like "nothing happened". A `RegisterHotKey` registration can go quietly dead in a long-lived process (still *held* — another registration fails with 1409 — but no `WM_HOTKEY` arrives; seen after two days uptime, fixed by a restart), and re-registering heals it without one. If PrtScn "does nothing", read `--status` first: no log line = the key never arrived; `overlay opening` with no `captured` = the capture itself failed.
...
- `MEMSNAP <a.snap> [b.snap] [summary|types|objects|labels|allocators|chapters] [top:N] [filter:X]` - Inspect a Unity Memory Profiler `.snap`, or diff two, **with no Unity Editor** — pure file parse, ~0.3s for a 140 MB capture. A folder argument summarises every `.snap` in it as one table. See [MemorySnapshot.cs](clibridge4unity/MemorySnapshot.cs).
  - Reports the memory-stats summary (total/reserved/graphics/GC), native objects by type and by name with counts, memory labels, allocators (used/reserved/peak), gfx resource total, managed heap size. Diffs show deltas with NEW/GONE flags.
  - The format is a chaptered container documented in the Memory Profiler package's own `FileReader.cs`, but **the on-disk chapter headers are not the 18 bytes its `EntryHeader` struct implies**: single = 18, const-array = 14 (count is a u32), dynamic-array = 10 + 8×(count+1) offsets with `offsets[0]=0` and `offsets[count]` = total. Headers are packed back-to-back; the package reads a fixed 18, overruns into the neighbour, and hides it by casting the count to `uint`. Reading the true widths avoids inheriting that. Blocks are shared: a single-element chapter sits at `HeaderMeta` bytes into its block.
  - `ProfileTarget_Info` uses natural (8-byte) alignment — it is 520 bytes on disk, not the struct's declared 512.
  - Chapter and block counts are read from the file, so captures from different Unity versions open with one reader; absent chapters have address 0. A capture taken with managed-only flags has no native chapters and correctly reports 0 native objects — use `chapters` to see what a file actually holds.
  - Pooled render textures (`TempBuffer N WxH`) are matched by dimensions, not index, so a pool rename does not masquerade as a 31 MB churn.
  - Deliberately does **not** crawl the managed heap: "N instances of Foo" means re-implementing the profiler's object crawler against per-version VM layouts. The native side is where leaks and regressions show first and is fully covered.
- `DEVICE list|install <apk|ipa>|uninstall|launch|log|tools` - The cable: adb (Android) + libimobiledevice (iOS). `install` picks the platform from the extension, `--launch` resolves the package/bundle id (aapt / Info.plist, else an installed-package diff, else `--package`), `log` is Unity-tagged logcat/syslog (`--follow`, `--all`). See [DeviceCommands.cs](clibridge4unity/DeviceCommands.cs).
  - **adb server rule:** one adb server per machine; a client of a different build silently kills and restarts it, which empties Ipaapk's / Unity's device list mid-session. So adb resolves as: `CLIBRIDGE_ADB` → **the binary behind the adb server already running** → Ipaapk's adb (`adb_override.txt`, then `%LOCALAPPDATA%\Ipaapk\current	ools`) → PATH → `ANDROID_HOME` / Android Studio / every Unity Hub editor's bundled SDK. Never `kill-server`.
  - libimobiledevice has no Windows installer; Ipaapk's `tools/` (complete, with DLLs — the exes aren't relocatable, so they run in place) is found first, then `CLIBRIDGE_IMOBILE_DIR`, PATH, scoop. `DEVICE tools` prints the resolution.
  - `.aab` is refused; iOS launch attaches `idevicedebug` and leaves it running (closing it kills the app).
  - **`DEVICE up <build> [--bp name] [--package id] [--wait s] [--no-install] [--no-launch]`** is the single entry point: install → log capture → launch → wait for the Bugpunch tunnel (matches the phone's model name, falls back to the only online internal device) → `session.json` + `~/.clibridge4unity/devices/.active`. Afterwards every DEVICE command defaults to that phone, and Unity-only verbs (`run`, `hierarchy`, `info`, `perf`, `prefs`, `memsnap`, `get`, `post`; `DEVICE bp <verb>` for the rest) route to the linked Bugpunch device with no device argument. `DEVICE status` · `DEVICE shell` (REPL, reads piped stdin) · `DEVICE down [--uninstall]`.
  - **Direct route (no server).** A Development Build serves the same Remote IDE API on the device (SDK `RemoteIDE/Connection/LocalIdeServer.cs`: `HttpListener` on 47701 feeding `BugpunchClient.ProcessRequestAsync`, UDP beacon on 47700; gated on `Debug.isDebugBuild || isEditor`, never a release). `DEVICE up` prefers it: `adb forward tcp:0 tcp:47701` / `iproxy <port> 47701 -u UDID` → probes `/` for the `local-ide` hello → `session.json.direct`; every Bugpunch verb then hits `127.0.0.1` with no token or role, re-establishing the forward if it died, falling back to the server route (`--via-server` forces it). `--lan <ip>` and `DEVICE discover` cover the network case. `BUGPUNCH <http-url> <verb>` addresses one directly. Needs SDK ≥ 0.8.223 in the build.
  - **iOS over USB.** iOS <= 16: the Developer Disk Image is fetched (xushuduo/Xcode-iOS-Developer-Disk-Image, once, into `~/.clibridge4unity/ddi`) and mounted before launch/screenshot — Ipaapk's approach. iOS 17+: the DDI is personalized and beyond libimobiledevice, so launch is manual and `DEVICE screenshot` falls back to the camera roll over AFC via Ipaapk's `afcshots.exe` (`--roll` latest, `--wait N` for one taken on request) — verified on the iPhone 11 Pro on iOS 26. Verified on the iPhone 11 Pro on iOS 26: `DEVICE up` installs the 690 MB MTD ipa in ~9 min, captures syslog, resolves the bundle id from `ideviceinstaller -l` by `.app` name (Info.plist is binary), reports launch as unavailable, and links Bugpunch once a token is stored.
  - **Native UI (Android):** `DEVICE screenshot` (screencap — native views included), `DEVICE ui [filter]` (uiautomator tree: center/clickable/text/id), `DEVICE tap "<text|id>"|x y`, `swipe`, `type`, `key`. This is the only way past a permission dialog / the SDK's consent sheet unattended — Bugpunch's capture and tap live inside the Unity surface. iOS is see-only (`idevicescreenshot`, needs the Developer Disk Image); native iOS taps need WebDriverAgent from a Mac.
  - **Session dir + `[artifacts]` footer** ([DeviceAutomation.cs](clibridge4unity/DeviceAutomation.cs)): every DEVICE/BUGPUNCH command runs in `~/.clibridge4unity/devices/<serial|id>/`, writes numbered artifacts (`0007_screenshot_x.png`, `ui_*.txt`, `run_*.txt`, `*.snap`), appends to `actions.log`, and ends stdout with `[artifacts] dir=… screenshot=… logcat=… (+N lines) actions=…` — the line a model reads to find the files. `DEVICE log --start|--stop|--status` runs a detached `adb logcat -T 1` / `idevicesyslog` into `logcat.txt`/`syslog.txt` (auto-started by `launch`, `install --launch`, `script`; never `logcat -c` — the buffer is shared with Ipaapk); the footer's `+N lines` is what arrived since the previous command.
  - **`DEVICE script <steps.txt> [--bp dev] [--continue] [--shots]`** — one command per line, unprefixed = DEVICE, `bp` = BUGPUNCH, plus `wait`, `waitfor "<text>" [sec]`, `assert`/`assert-not`, `bp waitonline [sec]`, `bp assert "<GameObject>"`. Stops at first failure, screenshots a failed assert, writes `NNNN_report_<name>.md` (step table + screenshots inline). This is the automation surface — same words as the CLI, no separate DSL.
- `BUGPUNCH auth <token> | devices [--all] | <device> run|action|tap|swipe|screenshot|log|hierarchy|info|perf|prefs|get|post` - The network: drive a game running the Bugpunch SDK through the server's personal-token API (`/api/v1/me`, `/api/v1/devices`, `/api/v1/devices/:id/proxy/*` — [apiV1Devices.routes.ts](../odddev/server/server/src/routes/apiV1Devices.routes.ts)). Aliases: `BP`.
  - **Internal devices only — enforced server-side.** The route lists/proxies only devices whose signed-in tester resolves to the `internal` role; external/public devices don't list and 404 on control, whatever the token. No client flag widens it.
  - Needs a personal access token minted with **Device control (admin)** (only global/org admins can — `scope: "admin"` on `POST /api/users/me/api-keys`). A read-scope token can `devices` but 403s on control. The token stays confined to `/api/v1/*` like every PAT; it never reaches dashboard routes. Stored in `~/.clibridge4unity/bugpunch.json`; `BUGPUNCH_TOKEN` / `BUGPUNCH_SERVER` override.
  - **`BUGPUNCH <dev> memsnap [list|pull <path>|delete <path>]`** — Unity Memory Profiler capture on the device (`/memory/snapshot`, Development Build only), polled to completion, pulled in 1 MB chunks via the SDK's `/memory/download` (added for this; the old `/files/read` path shipped the whole .snap as one base64 frame, which is what killed big captures in the tunnel), then `MEMSNAP summary` runs on the local file. The Remote IDE's MemoryPanel download uses the same chunked endpoint.
  - `<device>` = id, id prefix, or name/nickname/model substring; ambiguity is an error, never a guess. `run` posts `text/plain` C# to `/run`, `tap`/`swipe` are normalised 0–1 top-left, `screenshot` writes to `%TEMP%\clibridge4unity_screenshots\`. 403 `not_controller` = a dashboard user holds the Remote IDE lock.
