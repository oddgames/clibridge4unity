...
│   └── package.json           # UPM manifest (v1.1.85)
...
- **Grants — the gate asks once, not once per command.** Asking per command is unusable, not safe: one task can issue a dozen mutating commands. The dialog's *"Don't ask again while this play session lasts"* checkbox stores a grant scoped to `(window, playOwnerSince)`, so leaving play mode and re-entering invalidates it — permission given for one session never silently carries into the next. *"Always allow agents in this project"* (or `ALLOW <id> always`) writes a project-wide `grants/_always.txt` that survives play sessions **and conversations** until revoked — it is deliberately not per window, because a window id is its Claude process's pid and every new conversation is a new id, so a per-window "always" silently expired with the conversation. `REQUESTS` reports the standing permission; `REQUESTS --forget` revokes it; `CLIBRIDGE_NO_PLAYGATE=1` disables the gate for a window entirely.
